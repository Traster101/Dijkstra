				...
				content: [
					...
					["display-text",
						function() {return 'You have ' + formatWhole(player.q.energy)+' Quirk Energy ('+(tmp.nerdMode?('Base Gain: (timeInRun^(quirkLayers-1))'):'generated by Quirk Layers')+'), which multiplies Point and Generator Power gain by ' + format(tmp.q.enEff)+(tmp.nerdMode?(" ((x+1)^"+format(hasUpgrade("q", 23)?6:2)+"*"+format(improvementEffect("q", 23))+")"):"")},
							{}],
					['display-text', 
						() => `Your Quirk Energy/Second average is ${formatWhole(player[this.layer].energyPerSecond)} (${formatWhole(player.q.energyPerSecondMax)} max)`
					],
					"blank",
					"milestones", "blank",
					"blank",
...
addLayer("ss", {
		...
		update(diff) {
			...
			setPerSecond(player.ss, 'subspace')
		},
        row: 3, // Row the layer is in on the tree (0 is the first row)
        hotkeys: [
...
addLayer("ba", {
		...
		update(diff) {
			...
			setPerSecond(player.ba, 'pos')
			setPerSecond(player.ba, 'neg')
		},
...
addLayer("i", {
		...
		update(diff) {
			...
			setPerSecond(player.i, 'nb')
			player.i.hb = player.i.hb.max(tmp.i.hbAmt);
			setPerSecond(player.i, 'hb')
		},
		nbAmt() {
...
addLayer("ge", {
		...
		update(diff) {
			...
			setPerSecond(player.ge, 'gearSpeed')
...
addLayer("en", {
		...
		update(diff) {
			...
			if (hasMilestone("r", 1)) {
				...
				setPerSecond(player.en, 'sw', false)
				if (hasMilestone("en", 3)) {
					...
					setPerSecond(player.en, 'mw', false)
				}
...
addLayer("ne", {
		...
		update(diff) {
			if (player.ne.unlocked && (player.ne.activeChallenge==11||hasAchievement("a", 151))) {
				...
				setPerSecond(player.ne, 'signals')
