...
// data = a function returning the content (actually HTML)
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			...
			<br><br>
			<span v-if="tmp[layer].resetGain != null">Your current gain/sec average since this last row reset is {{format(tmp[layer].resetGainPerSecond)}} ({{format(tmp[layer].resetGainPerSecondMax)}} max)<br></span>
		...
		<div class="popup" v-if="!(!player.saveMenuOpen)">
			...
			<div class="popup-content"><br>
				<div class="upgRow" style="width: 100%;" v-for="(data, name) in allSaves" v-if="name!='set' && data!==undefined">
					<div v-bind:class="{ activeSave: (allSaves.set==name), widthLock: true }">{{name}} {{formatStopwatch(allSaves[name].timePlayed)}}</div>
